...
const middle2 = `	len(encTableBytes),
}
...
func main() {
	...
	idxs := make(map[string]int, len(encodingTable))
	idx := 0
	...
	for _, name := range encodingTable {
		...
		case 2:
			fmt.Fprintf(out, "\t0x%02x, 0x%02x, // %s\n", buf[0], buf[1], name)
		case 3:
			fmt.Fprintf(out, "\t0x%02x, 0x%02x, 0x%02x, // %s\n", buf[0], buf[1], buf[2], name)
		case 4:
			fmt.Fprintf(out, "\t0x%02x, 0x%02x, 0x%02x, 0x%02x, // %s\n", buf[0], buf[1], buf[2], buf[3], name)
		default:
			panic("unreachable")
		}
	}

	fmt.Fprint(out, middle1)

	for _, name := range encodingTable {
		fmt.Fprintf(out, "\t0x%04x, // %s\n", idxs[name], name)
	}

	fmt.Fprint(out, middle2)
